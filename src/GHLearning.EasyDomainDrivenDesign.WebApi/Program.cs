...
builder.Services
	.AddInfrastructure((sp, options) =>
	{
		var mongoDbConnectionString = builder.Configuration.GetConnectionString("MongoDB")
			?? throw new InvalidOperationException("MongoDB connection string is not configured.");
		options.UseMongoDB(mongoDbConnectionString, "easy-domain-driven-design");
	})
	.AddApplication();
...
{
	x.AddConsumer<AnnouncementPublishedConsumer>();
	x.AddConsumer<AnnouncementLogConsumer>();

	x.UsingRabbitMq((context, cfg) =>
	{
		cfg.Host(builder.Configuration.GetConnectionString("RabbitMQ"));

		cfg.ReceiveEndpoint("announcement-published", e =>
		{
			e.ConfigureConsumer<AnnouncementPublishedConsumer>(context);
			e.UseMessageRetry(r => r.Incremental(3, TimeSpan.FromSeconds(10), TimeSpan.FromSeconds(10)));
			e.DiscardFaultedMessages();
		});

		cfg.ReceiveEndpoint("announcement-log", e =>
		{
			e.ConfigureConsumer<AnnouncementLogConsumer>(context);
			e.UseMessageRetry(r => r.Incremental(3, TimeSpan.FromSeconds(10), TimeSpan.FromSeconds(10)));
			e.DiscardFaultedMessages();
		});
	});
